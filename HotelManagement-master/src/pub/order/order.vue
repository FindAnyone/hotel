    ...
    <div id="roomstate">
        ...
        >
            ...
            <el-table-column label="操作" width="150">
                <template slot-scope="scope">
                    <el-button type="text" icon="el-icon-edit" @click="$refs.Checkin.open(orderData)" v-show="scope.row.state == 1">入住</el-button>
                    <el-button type="text" icon="el-icon-edit" @click="$refs.Checkininfo.open(orderData)" v-show="(scope.row.state == 2)">查看</el-button>
                    <el-button type="text" icon="el-icon-edit" @click="checkout" v-show="(scope.row.state == 2)">退房</el-button>
                    <el-button type="text" icon="el-icon-delete" class="red" @click="handleDelete" v-show="(scope.row.state == 3)">同意取消</el-button>
                </template>
            </el-table-column>
        </el-table>
        <Checkin ref="Checkin" title="入住" @confirmData="(item) => checkin(item)" />
        <Checkininfo ref="Checkininfo" title="入住" @confirmData="(item) => checkininfo(item)" />
        <PageComponent :total="page.totalSize" :page="page" @pageChange="handlePageChange" />
    </div>
</template>

<script scoped>
import { getorderList, delorder, checkout, getcancelOrderConfirm } from '@/api/order';
    ...
    methods: {
        ...
        checkin(item) {
            ...
            for (let i = 0; i < item.domains.length; i++) {
                this.checkinList.push({
                    ...
                    checkName: item.domains[i].checkName,
                    checkIdCard: item.domains[i].checkIdCard
                });
            }
            const param = this.checkinList;
            console.log(param)
            addcheckin({ param }).then((res) => {
        ...
        checkout() {
            this.$confirm('是否退房？', '提示', {
                ...
                .then((res) => {
                    checkout(this.orderData).then((res) => {
                        if (res.data.code == 2000) {
                            this.$message({
                                type: 'success',
                                message: '退房成功'
